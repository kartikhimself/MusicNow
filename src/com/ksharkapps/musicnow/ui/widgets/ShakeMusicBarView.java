...
public class ShakeMusicBarView extends LinearLayout {
	private List<ShakeMusicBarViewSingle> mBars = new ArrayList<ShakeMusicBarViewSingle>();
	...
	private void createBars(int count){
		...
		for(int i =0;i<count;i++){
			mBars.add(new ShakeMusicBarViewSingle(getContext(), mHandler));
		}
	}
		...
		this.post(new Runnable() {
			...
			public void run() {
				...
				for(int i=0;i<mBars.size();i++){
					ShakeMusicBarViewSingle currentBar = mBars.get(i);
	...
	public void shake(boolean isShake){
		for(ShakeMusicBarViewSingle bar : mBars){
			bar.shake(isShake);
		}
	}
